...
/*   Updated: 2020/07/22 00:55:18 by white            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_format_print(va_list ap, int j, char *format, t_param *f_param)
{
	if (format[j] == 'c')
		...
		print_hex(for_u_x(ap, f_param), f_param, "0123456789abcdef", 0, "0x");
	else if (format[j] == 'X')
		print_hex(for_u_x(ap, f_param), f_param, "0123456789ABCDEF", 0, "0X");
	else if (format[j] == 'p')
		print_hex(va_arg(ap, unsigned long int), f_param, "0123456789abcdef", 1, "0x");
